...
# Initialize Chroma client with disk-backed storage path
...
def index_website_content(all_texts):
    ...
    embeddings = [get_embedding(chunk) for chunk in tqdm(all_texts)]

    # Create a collection to store the embeddings and metadata (URLs)
    collection = client.create_collection("web_pages")

    # Add the embeddings and corresponding metadata (URLs) to Chroma
    for i, chunk in enumerate(all_texts):
        collection.add(
            documents=[chunk],
            metadatas=[{"url": all_urls[i]}],  # Store URLs as metadata
            embeddings=[embeddings[i]]  # The embeddings themselves
        )
...
def load_chroma_index():
    """Loads the Chroma index from disk."""
    return client.get_collection("web_pages")

def generate_answer(query, collection):
    """Uses Azure OpenAI to generate an answer based on retrieved context."""
    # Perform a similarity search in Chroma for relevant documents
    query_embedding = get_embedding(query)
    results = collection.query(query_embeddings=[query_embedding], n_results=5)

    # Extract the relevant chunks and their URLs
    relevant_chunks = []
...
def main():
    ...
    # Step 1: Load the Chroma index if the environment variable is set
    if LOAD_EXISTING_INDEX:
        print("Loading existing Chroma index...")
        collection = load_chroma_index()
    else:
        ...
        for url in all_urls:
            ...
            texts = scrape_website(url)  # Use the existing scrape function
            if texts:
                all_texts_with_urls.extend(zip(texts, [url]*len(texts)))  # Include URLs in chunks
        
        # Step 3: Create Chroma index from scraped content
        print("Indexing content...")
        collection = index_website_content(all_texts_with_urls)
        clent.save_collection(collection)

    # Step 4: Answer a question based on the indexed content
    question = "What is the main topic of the website?"
    answer = generate_answer(question, collection)
    print("Answer:", answer)
